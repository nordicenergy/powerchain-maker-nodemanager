...
func (nsi *NodeServiceImpl) IPWhitelister() {
	go func() {
		if _, err := os.Stat("/root/powerchain-maker/contracts/.whiteList"); os.IsNotExist(err) {
			util.CreateFile("/root/powerchain-maker/contracts/.whiteList")
		}
		whitelistedIPs, _ := util.File2lines("/root/powerchain-maker/contracts/.whiteList")
...
func (nsi *NodeServiceImpl) GetGenesisHandler(w http.ResponseWriter, r *http.Request) {
	...
	if allowedIPs[foreignIP] {
		...
		if exists != "" {
			go func() {
				b, err := ioutil.ReadFile("/root/powerchain-maker/JoinRequestTemplate.txt")
