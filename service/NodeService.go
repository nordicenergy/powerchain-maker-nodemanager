...
func (nsi *NodeServiceImpl) DirectoryCrawl() {
	abiCrawlerMutex = 1
	ABIList := getFilesFromDirectory("/root/powerchain-maker/contracts")
...
func getLastCheckedTime() int64 {
	fileBytes, err := ioutil.ReadFile("/root/powerchain-maker/contracts/.lastCheckedTime")
	if err != nil {
		log.Println(err)
	}
	jsonContent := string(fileBytes)
...
func updateLastCheckedTime(timeVal string) {
	util.DeleteFile("/root/powerchain-maker/contracts/.lastCheckedTime")
	util.CreateFile("/root/powerchain-maker/contracts/.lastCheckedTime")
	util.WriteFile("/root/powerchain-maker/contracts/.lastCheckedTime", timeVal)
}
